...
UpCrossSelling.App = (function ($, window, document, undefined) {
		...
		$(document).on('click', '.notifyjs-foo-base .yes', function() {
		  /*show button text*/
			$.ajax({ 
				type: "POST",
				data: {id: comment_id},
		       	url: "CommentsController.do?action=delete_comment",   
	...
	{
		$.ajax({ type: "GET",   
		       url: "OpportunityController.do?action=list_notes&IDUpCrossSelling=" + idParentItem,   
		       success : function(data){
		    	   DrawComments(data)
		       },
	        error: function(error){
	...
	var CreateComment = function(idselling, type)
	{
		...
		if(comment_ != '')
		{
			$.ajax({ 
				type: "POST",
				data: {parentid: idselling, tableName: type, comment: comment_},
		       	url: "CommentsController.do?action=create_comment",   
		       	success : function(data){
